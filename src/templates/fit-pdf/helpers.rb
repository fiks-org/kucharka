...
module WooWoo
  module Helpers
    ...
    TEMPLATE_SIGNATURE = 'FIT PDF Template v0.2.3'.freeze
    ...
    #
    # @param idx_labels [String,Array]
    #
    def index_macro(idx_labels)
      if idx_labels.is_a?(String)
        "\\index{#{idx_labels}}"
      elsif idx_labels.is_a?(Array)
        idx_labels.map{ |item| "\\index{#{item}}"}.join
      end
    end
