...
namespace :fikshtml do
  desc 'Check out the document spine.'
  task :spine do
    template = WooWoo::Template.new('fiks-html')
  ...
  task :assets do
    ...
    sh "cp #{File.join('templates', 'fiks-html', 'css', '*')} #{File.join(builder.template_build_dir, 'css')}/"
    sh "cp #{File.join('templates', 'fiks-html', 'js', '*')} #{File.join(builder.template_build_dir, 'js')}/"
